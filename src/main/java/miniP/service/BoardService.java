...
public class BoardService {
    ...
    // 게시글 저장
    @Transactional
    public BoardResponseDto save(BoardRequestDto boardRequestDto, HttpServletRequest request) throws RuntimeException {
        try {
...
//            String username2 = validateToken(request);
            ...
            BoardResponseDto boardResponseDto = board.toDto(board);
    ...
    public BoardResponseDto getOne(Long id) throws RuntimeException {
        try {
            Board getBoard = boardRepository.findById(id).orElseThrow(() -> new RuntimeException(" 게시물을 찾을 수 없습니다 "));
            BoardResponseDto boardResponseDto = getBoard.toDto(getBoard);
            return boardResponseDto;
        } catch (RuntimeException e) {
